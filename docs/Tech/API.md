...
Не требует входа. Возвращает выбранную администратором тему водительской страницы и сохранённый основной набор текстов (для русского языка):
...
Водитель выбирает язык на публичной странице. Для русского языка frontend использует сохранённый основной набор, для остальных языков — встроенный штатный перевод, если отдельные значения не заданы.
...
Администраторская форма использует этот endpoint только с `language=ru`: она редактирует основной текстовый набор, а выбор языка остаётся на публичной водительской странице. Endpoint сохраняет совместимость с ранее созданными локализованными значениями.
