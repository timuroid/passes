...
`false` восстанавливает запись. Операция не удаляет строку из БД.
...
Только `admin`; требует `X-CSRF-Token`. Создаёт активную учётную запись в локальной БД.
...
Проверяет соединение с БД и возвращает `{"status":"ok"}`.
